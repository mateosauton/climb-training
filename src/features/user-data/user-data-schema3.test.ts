...
function user(id: string, subject: string | null) {
  return {
    identity: {
      ...
      createdAt: at,
      updatedAt: at,
      auth: subject ? { provider: "supabase", subject, email: `${id}@example.com` } : null
    },
    facts: [],
    sessionLogs: [],
    videoAnalyses: [],
...
function schema3() {
  return {
    schemaVersion: 3,
    activeUserId: "u1",
    users: { u1: user("u1", "user-1"), u2: user("u2", null) },
    migration: { migratedFrom: "climb4w.users.v2", migratedAt: at }
  };
}

describe("user data schema 3", () => {
  it("accepts Supabase and unbound identities", () => {
    const value = schema3();
    expect(validateUserDataEnvelope(value)).toEqual(value);
  });

  it("rejects malformed and duplicate Supabase subjects", () => {
    ...
    duplicate.users.u2.identity.auth = { provider: "supabase", subject: "user-1", email: null };
